 ...
 */
/**
 * Processor which handles the tag whcih are specific to a VALUEBLOCK.
 * 
 * @author par7kor
 * 
 */
public class ValBlkProcessor implements ITagProcessor {

	/**
	 * ValBlkCategoryAdapter instance.
	 */
	private AbstractValBlkCategoryAdapter valBlkCategoryAdapter;
	 ...
	 * List which hold the values(WERT) got from paco file.
	 */
	private final List wertList = new ArrayList();
	 ...
	 */
	private static final String CLASSNAME = "com.bosch.calcomp.pacoparser.tagprocessor.ValBlkProcessor";
	 ...
	 */
	public ValBlkProcessor(final PacoParser pacoParser) throws PacoParserException {
		LoggerUtil.getLogger().debug(CLASSNAME+LOGMSGDELIMITER+"ValBlkProcessor Constructor started.");
		if (this.valBlkCategoryAdapter == null) {
			...
			ICategoryAdapter categoryAdapter = modelAdapterFactory
					.createValBlkAdapter(pacoParser);
			this.valBlkCategoryAdapter = (AbstractValBlkCategoryAdapter) categoryAdapter;
		}
		LoggerUtil.getLogger().debug(CLASSNAME+LOGMSGDELIMITER+"ValBlkProcessor Constructor ended.");
	}

	/**
	 * Processes the valBlk specific tags and sets to valBlk adpater.
	 * 
	...
	public final void process(final String currTagName, final String dataString)
		...
		if (this.valBlkCategoryAdapter != null) {
			try {
//				setUnits(currTagName, dataString);
...
//                SwUnitsCreator.getInstance().addToUnitsMap(currTagName, dataString)
                ...
                valBlkCategoryAdapter.setUnit(dataString, swUnitsCreator);
                ...
                valBlkCategoryAdapter.setTextFlag(true);
                ...
                valueList.add(dataString);
                valBlkCategoryAdapter.setTextFlag(false);
            }
            setValueList(currTagName);
   ...
   * @throws PacoParserException
   */
  private void setUnits(final String currTagName, final String dataString) throws PacoParserException {
    if (PacoFileTagNames.SW_UNIT_REF.equals(currTagName)) {
        new SwUnitsCreator().addToUnitsMap(currTagName, dataString);
    	this.valBlkCategoryAdapter.setUnit(dataString);
    } else if (PacoFileTagNames.SW_AXIS_INDEX.equals(currTagName)) {
    	axisIndex = Integer.parseInt(dataString);
    } else if (PacoFileTagNames.VT.equals(currTagName)) {
    	// value is in the form of text therefore setTextFlag is set
    	...
    	valueList.add(dataString);
    	this.valBlkCategoryAdapter.setTextFlag(true);
    } else if (PacoFileTagNames.V.equals(currTagName)) {
    	// value is in numerical format.
    	if (valueList == null) {
    		valueList = new ArrayList();
    	...
    	valueList.add(dataString);
    	this.valBlkCategoryAdapter.setTextFlag(false);
    }
  }

	/**
	 * Function which set the values to the valBlk adapter.
	 * 
	 * @param tagName -
	 *            current tag name.
	 * @throws PacoParserException -
	...
	public final void setValueList(final String tagName)
		...
		if (this.valBlkCategoryAdapter != null) {
			...
			if (PacoFileTagNames.SW_INSTANCE.equalsIgnoreCase(tagName)) {
				this.valBlkCategoryAdapter.setPacoData();
			}
		}
   ...
   * @throws PacoParserException
   */
  private void setAttr(final String tagName) throws PacoParserException {
    if (PacoFileTagNames.VG.equalsIgnoreCase(tagName)) {
    	...
    	if (axisIndex == 0) { // WERT
    		...
    		this.valBlkCategoryAdapter.setValues(wertList);
    	}
    	valueList = null;
    }
  }
	...
	/**
	 * Gets the valBlk category adapter instance.
	 * 
	 * @return ICategoryAdapter
	 */
	public final ICategoryAdapter getCategoryAdapter() {
		return this.valBlkCategoryAdapter;
	}
