 ...
 * @author mkl2cob
 */
public class CompHEXCDFEditConfiguration extends AbstractRegistryConfiguration {
  ...
  public void configureRegistry(final IConfigRegistry configRegistry) {
    ...
    ImagePainter imgPainter = new ImagePainter() {
      ...
      protected Image getImage(final ILayerCell cell, final IConfigRegistry configRegistry) {
        ...
        CustomFilterGridLayer<CompHexWithCDFParam> customFilterGridLayer =
            (CustomFilterGridLayer<CompHexWithCDFParam>) cell.getLayer();
        CompHexWithCDFParam rowObj = customFilterGridLayer.getBodyDataProvider().getRowObject(cell.getRowIndex());
        String paramType = rowObj.getParamType().getText();
    ...
    // multi image painter to display multiple image in single cell of nat table
    ...
    // if the fucntion versions are different
    ...
    configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_STYLE, colorYellow, DisplayMode.NORMAL,
        CDRConstants.FUNC_VERS_DIFF);


    // Color code for Review Score column
    // Zero Rating
    ...
    // Create a new style for  review result hyperlinks
    ...
    // Register the hyperlink style with the configuration registry for use in NORMAL display mode
