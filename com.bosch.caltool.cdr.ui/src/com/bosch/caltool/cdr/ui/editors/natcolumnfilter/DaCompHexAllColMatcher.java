 ...
 * @author mkl2cob
 */
public class DaCompHexAllColMatcher<E extends DaCompareHexParam> implements Matcher<E> {
  ...
  /**
   * @param dataAssmntReportDataHandler handler
   */
  public DaCompHexAllColMatcher(final DataAssmntReportDataHandler dataAssmntReportDataHandler) {
  ...
  private boolean matchCompliResult(final E arg0) {
    return matchText(this.dataAssmntReportDataHandler.getResult(arg0));
  }

  private boolean matchWp(final E arg0) {
    return matchText(arg0.getWpName());
  }

  private boolean matchRespType(final E arg0) {
    return matchText(CommonUtils.isEqual(WpRespType.RB, WpRespType.getType(arg0.getRespType())) ||
        CommonUtils.isEqual(WpRespType.CUSTOMER, WpRespType.getType(arg0.getRespType())) ||
        CommonUtils.isEqual(WpRespType.OTHERS, WpRespType.getType(arg0.getRespType()))
            ? WpRespType.getType(arg0.getRespType()).getDispName() : arg0.getRespType());
  }
  ...
  private boolean matchResp(final E arg0) {
    return matchText(arg0.getRespName());
  }
  ...
  private boolean matchHEXCalData(final E arg0) {
    String calDataString = this.dataAssmntReportDataHandler.getCalDataStringFromBytes(arg0.getHexValue());
    if (CommonUtils.isEmptyString(calDataString)) {
      return false;
    }
    return matchText(calDataString);
  }
  ...
  private boolean matchCDFCalData(final E arg0) {
    String calDataString = this.dataAssmntReportDataHandler.getCalDataStringFromBytes(arg0.getReviewedValue());
    if (CommonUtils.isEmptyString(calDataString)) {
      return false;
    }
    return matchText(calDataString);
