--------------------------------------------------------
--  Datei erstellt -Sonntag-Mai-24-2020   
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package PK_A2L_WP_DETAILS_COPY
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "PK_A2L_WP_DETAILS_COPY" AS 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE "PK_GROUP2PAL" AS 
  ...
  -- create version 2
  --
  procedure MigrateGroup2PalWP(pPidcA2LID IN NUMBER, pCreUser VARCHAR2);
  ...
  --
  --
  procedure SetRvwRefs2DefaultWp(pPidcA2LID IN NUMBER, pCreUser   IN VARCHAR2);

  --
  --
  --
  procedure InitData(pPidcVersID IN NUMBER, pSdomPverName IN VARCHAR2, pCreUser IN VARCHAR2);

  --
  --
  --
  function CreateNewPalWPVersion(pPidcA2LID IN NUMBER, pCreUser VARCHAR2) return number;
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PK_A2L_WP_DETAILS_COPY" AS
    
	    
	-- this procedure does not have a commit statement. commit is happening via the command.  Also the T_A2L_WP_DEFN_VERSIONS record is already created
    procedure InsertA2lWpDetails(p_source_wp_def_vers_id IN NUMBER, p_dest_wp_def_vers_id IN NUMBER, p_created_user_name VARCHAR2) 
    AS
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "PK_GROUP2PAL" AS
