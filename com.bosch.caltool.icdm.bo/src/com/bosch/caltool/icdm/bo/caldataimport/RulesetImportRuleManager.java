 ...
 * @author jvi6cob
 */
public class RulesetImportRuleManager extends AbstractImportRuleManager {

  /**
   * @param ssdServiceHandler {@link SSDServiceHandler}
   */
  public RulesetImportRuleManager(final ServiceData serviceData, final SSDServiceHandler ssdServiceHandler) {
  ...
  public Map<CDRRule, SSDMessage> updateRules(final ParamCollection calDataImporterObject,
    ...
    Map<CDRRule, SSDMessage> cdrRuleSsdMsgMap = getServiceHandler().updateValidMultRuleSetRules(oldCdrList,
        ((RuleSet) calDataImporterObject).getSsdNodeId(), calDataImporterObject.getId());

    Map<String, List<CDRRule>> newCdrMapUpdated =
        getServiceHandler().readSSDRule(labelNames, ((RuleSet) calDataImporterObject).getSsdNodeId());
  ...
  public Map<CDRRule, SSDMessage> createRules(final ParamCollection calDataImporterObject,
    ...
    RuleSet ruleSet = (RuleSet) calDataImporterObject;
    createRuleObjects(importData, calDataImporterObject, listOfRulesToBeCreated);
    if (!listOfRulesToBeCreated.isEmpty()) {
      createMsg = handler.createValidMultRuleSetRules(listOfRulesToBeCreated, ruleSet.getSsdNodeId(),
  ...
  public void readRules(final SSDServiceHandler ssdServiceHandler, final CalDataImportData importData,
    ...
    RuleSetRulesResponse paramRuleResponse = fetcher.createRuleSetRulesOutput(calDataImporterObject.getId());

    Map<String, List<CDRRule>> ssdRuleMap = new HashMap<>();
    if(!allParamList.isEmpty()) {
    ssdRuleMap =
        ssdServiceHandler.readReviewRule(allParamList, ((RuleSet) calDataImporterObject).getSsdNodeId());
    }
    ...
    for (Entry<String, Map<Integer, Set<AttributeValueModel>>> commonCombiAttrValModelEntry : commonCombiAttrValModel
      ...
      RuleSetParameter param = paramRuleResponse.getParamMap().get(commonCombiAttrValModelEntry.getKey());
  ...
  private void readRulesWithoutCombinations(final CalDataImportData importData,
      final Map<String, List<CDRRule>> ssdRuleMap, final RuleSetRulesResponse paramRuleResponse,
      final List<String> paramList, final RuleSetParameter parameter)
      throws IcdmException {
    ...
    if (!ssdRuleWithoutDependenciesMap.isEmpty()) {
      for (Entry<String, List<CDRRule>> ssdRuleMapEntry : ssdRuleWithoutDependenciesMap.entrySet()) {
        ...
        for (CDRRule cdrRul : ruleList) {
          ...
          List<RuleSetParameterAttr> parmAttrList = paramRuleResponse.getAttrMap().get(parameter.getName());
  ...
  private void readRuleWithCombinations(final CalDataImportData importData, final Map<String, List<CDRRule>> ssdRuleMap,
      final RuleSetRulesResponse paramRuleResponse, final Map<Integer, Set<FeatureValueModel>> feaValCombiModels,
      final RuleSetParameter parameter, final List<String> paramList)
      throws IcdmException {
  ...
  private void consolidateRulesForSpecficCombination(final CalDataImportData importData, final List<String> paramList,
      final RuleSetRulesResponse paramRuleResponse, final Map<String, List<CDRRule>> consolidatedParamRuleListMap,
      final RuleSetParameter parameter, final Entry<Integer, Set<FeatureValueModel>> featureValCombiEntry)
      throws IcdmException {
