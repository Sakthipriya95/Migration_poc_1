/*
 * Copyright (c) Robert Bosch GmbH. All rights reserved.
 */
package com.bosch.caltool.apic.ui.sorter;

import org.eclipse.jface.viewers.Viewer;

import com.bosch.caltool.icdm.client.bo.apic.AliasDefEditorDataHandler;
import com.bosch.caltool.icdm.common.ui.utils.CommonUIConstants;
import com.bosch.caltool.icdm.common.util.ApicUtil;
import com.bosch.caltool.icdm.model.apic.ApicConstants;
import com.bosch.caltool.icdm.model.apic.attr.AttributeValue;
import com.bosch.rcputils.sorters.AbstractViewerSorter;


/**
 * @author rgo7cob
 */
public class AliasValueSorter extends AbstractViewerSorter {

  /**
   * Represents the column number to be sorted
   */
  private int index;

  /**
   * Default diraction ascending
   */
  private int direction = ApicConstants.ASCENDING;


  private final AliasDefEditorDataHandler dataHandler;

  /**
   * @param dataHandler Alias Def Editor Data Handler
   */
  public AliasValueSorter(final AliasDefEditorDataHandler dataHandler) {
    this.dataHandler = dataHandler;
  }

  /**
   * index defines grid tableviewercolumn index {@inheritDoc}
   */
  @Override
  public void setColumn(final int index) {
    if (index == this.index) {
      this.direction = 1 - this.direction;
    }
    else {
      this.index = index;
      this.direction = ApicConstants.ASCENDING;
    }

  }

  /**
   * Compares two pid's details {@inheritDoc}
   */
  @Override
  public int compare(final Viewer viewer, final Object obj1, final Object obj2) { // NOPMD by bne4cob on 3/11/14 12:16
    int result = 0;
    // check pidc instance
    if ((obj1 instanceof AttributeValue) && (obj2 instanceof AttributeValue)) {
      AttributeValue result1 = (AttributeValue) obj1;
      AttributeValue result2 = (AttributeValue) obj2;

      switch (this.index) {
        // compare attr value name
        case CommonUIConstants.COLUMN_INDEX_0:
          result = ApicUtil.compare(result1.getName(), result2.getName());
          break;
        case CommonUIConstants.COLUMN_INDEX_1:
          result = compareAliasName(result1, result2);
          break;
        default:
          // if no column available
          result = 0;
          break;
      }
      // If descending order, flip the direction
      if (this.direction == ApicConstants.DESCENDING) {
        result = -result;
      }
    }
    return result;
  }

  /**
   * @param result1
   * @param result2
   * @return
   */
  private int compareAliasName(final AttributeValue result1, final AttributeValue result2) {

    // Compare alias name
    return ApicUtil.compare(this.dataHandler.getValueAliasName(result1.getId()),
        this.dataHandler.getValueAliasName(result2.getId()));

  }


  /**
   * @return defines direction of sort
   */
  @Override
  public int getDirection() {
    return this.direction;
  }


}
