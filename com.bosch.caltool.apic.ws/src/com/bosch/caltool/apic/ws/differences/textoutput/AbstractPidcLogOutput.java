...
import com.bosch.caltool.apic.ws.attribute.adapter.AttributeAdapter;
import com.bosch.caltool.apic.ws.attribute.adapter.AttributeValueAdapter;
 ...
 * @author imi2si
 */
public abstract class AbstractPidcLogOutput implements IWebServiceResponse {

  protected final GetPidcDiffsResponseType diff;
   ...
   * @param language
   */
  public AbstractPidcLogOutput(final ILoggerAdapter logger, final ServiceData serviceData,
      final GetPidcDiffsResponseType diff, final String language) {
    this.diff = diff;
    this.logger = logger;
  ...
  public Attribute getAttribute(final Long attributeID) throws IcdmException {
    ...
    return new AttributeAdapter(this.logger, attrLoader.getDataObjectByID(attributeID));
  }

  public boolean isAttributeExisting(final Long attributeID) {
    try {

      return getAttribute(attributeID).getNameE() != null;
    }
    catch (Exception e) {
  ...
  public AttributeValue getAttrValue(final Long valueID) throws IcdmException {
    ...
    return ((null != valueID) && (valueID > 0))
        ? new AttributeValueAdapter(this.logger, attrValueLoader.getDataObjectByID(valueID)) : null;
  }
  ...
  protected String getUserName(final String userName) {
  ...
  public boolean equals(final Object obj) {
    ...
    AbstractPidcLogOutput other = (AbstractPidcLogOutput) obj;
    if (this.diff == null) {
      if (other.diff != null) {
        return false;
      }
