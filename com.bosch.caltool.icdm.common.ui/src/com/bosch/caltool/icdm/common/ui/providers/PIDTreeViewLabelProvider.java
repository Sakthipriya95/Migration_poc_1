...
/**
 * This class provides the labels for ProjectIDCard tree viewer
 *
 * @author adn1cob
 */
public class PIDTreeViewLabelProvider extends AbstractLabelProvider {
  ...
  private final PidcTreeNodeHandler treeHandler;
   ...
   * Constructor
   *
   * @param treeHandler Pidc Tree View Data Handler
   */
  public PIDTreeViewLabelProvider(final PidcTreeNodeHandler treeHandler, final TreeViewer treeViewer) {
    ...
    this.styler = new Styler() {
      ...
      public void applyStyles(final TextStyle textStyle) {
        textStyle.font = PIDTreeViewLabelProvider.this.italicFont;
      }
    };
  }
    ...
    if (element instanceof PidcTreeNode) {
      ...
      if (pidcNode.getNodeType().equals(PIDC_TREE_NODE_TYPE.LEVEL_ATTRIBUTE) &&
        ...
        nodeImg = ImageManager.getInstance().getRegisteredImage(ImageKeys.PIDC_ROOT_28X30);
          ...
          (pidcNode.getLevel() > ApicConstants.PIDC_ROOT_LEVEL)) {
        nodeImg = this.treeHandler.hasPidCardsBelow(pidcNode)
            ? ImageManager.getInstance().getRegisteredImage(ImageKeys.PIDC_NODE_28X30) : null;
      }
        ...
        nodeImg = setNodeImageForPIDC(pidcNode);
      }
  ...
  private Image checkAndAddNodeImages(final ViewerCell cell, Image nodeImg, final PidcTreeNode pidcNode) {
    if (pidcNode.getNodeType().equals(PIDC_TREE_NODE_TYPE.PIDC_A2L_QNAIRE_RESP_NODE)) {
    ...
    else if (pidcNode.getNodeType().equals(PIDC_TREE_NODE_TYPE.QNAIRE_RESP_NODE)) {
  ...
  private boolean isWpFinished(final PidcTreeNode pidcNode, final Long pidcVariantId, final Long a2lRespId,
    ...
    Map<Long, Map<Long, String>> pidcVarWpRespMap = wpRespStatusMap.get(pidcVariantId);

    return wpRespStatusMap.containsKey(pidcVariantId) && pidcVarWpRespMap.containsKey(a2lRespId) &&
        pidcVarWpRespMap.get(a2lRespId).containsKey(a2lWpId) &&
        isWPComplete(pidcVarWpRespMap.get(a2lRespId).get(a2lWpId));
  }
  ...
  private boolean isA2lRespFinished(final PidcTreeNode pidcNode, final Long pidcVariantId, final Long a2lRespId) {
    ...
    Map<Long, Map<Long, Map<Long, String>>> wpRespStatusMap = pidcNode.getA2lStructureModel().getWpRespStatusMap();
    Map<Long, Map<Long, String>> pidcWpRespMap = wpRespStatusMap.get(pidcVariantId);

    return wpRespStatusMap.containsKey(pidcVariantId) && pidcWpRespMap.containsKey(a2lRespId) &&
        isRespComplete(pidcWpRespMap.get(a2lRespId));
  }
   ...
   * @return
   */
  private Image setNodeImageForPIDC(final PidcTreeNode pidcNode) {
    Image nodeImg;
  ...
  /**
   * Special handling of dummy variant nodes
   * <p>
   * {@inheritDoc}
   */
  @Override
  public String getToolTipText(final Object element) {
    ...
    if (element instanceof PidcTreeNode) {
      fetchToolTip(element, toolTipText);
    }
   ...
   * @param toolTipText
   */
  private void fetchToolTip(final Object element, final StringBuilder toolTipText) {
    PidcTreeNode treeNode = (PidcTreeNode) element;
    if (treeNode.getNodeType().equals(PidcTreeNode.PIDC_TREE_NODE_TYPE.LEVEL_ATTRIBUTE)) {
      getToolTipForLevelAttribute(toolTipText, treeNode);
    }
    ...
    if (treeNode.getNodeType().equals(PIDC_TREE_NODE_TYPE.CDR_VAR_NODE)) {
      this.tooltipUtil.getCdrVarToolTip(toolTipText, treeNode);
    }
    if (treeNode.getNodeType().equals(PIDC_TREE_NODE_TYPE.RVW_OTHER_RVW_SCOPES_TITLE_NODE)) {
      this.tooltipUtil.getOtherScopeToolTip(toolTipText);
    }
  }
   ...
   * @param treeNode
   */
  private void getToolTipForLevelAttribute(final StringBuilder toolTipText, final PidcTreeNode treeNode) {
    if (treeNode.getLevel() == 0) {
      this.tooltipUtil.getAttrDetailsTooltip(toolTipText, treeNode);
    }
    this.tooltipUtil.getAttrDetailsTooltip(toolTipText, treeNode);
    this.tooltipUtil.getValDetailsTooltip(toolTipText, treeNode);
  }
