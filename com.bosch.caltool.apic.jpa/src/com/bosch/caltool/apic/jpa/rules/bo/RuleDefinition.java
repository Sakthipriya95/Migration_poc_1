...
public class RuleDefinition implements Comparable<RuleDefinition> {
   ...
   */
  private Map<String, CDRRule> ruleMapping = new HashMap<String, CDRRule>();
  /**
   * THis map has the column index as key and maturity level as value
   */
  private final Map<Integer, String> ruleMappedIndex = new HashMap<Integer, String>();
  /**
   * Reference Value
   */
   ...
   */
  public Map<String, CDRRule> getRuleMapping() {
   ...
   */
  public void setRuleMapping(final Map<String, CDRRule> ruleMapping) {
  ...
  private boolean ruleDefnEquals(final Object obj) {
    ...
    if (valueType(ruleDefn) || complexType(ruleDefn)) {
      return true;
    }
    return false;
  }
   ...
   * @return
   */
  private boolean valueType(final RuleDefinition ruleDefn) {
    return ruleDefn.getValueType().equalsIgnoreCase(VALUE_TEXT) &&
        CommonUtils.isEqual(ruleDefn.getLowerLimit(), getLowerLimit()) &&
        CommonUtils.isEqual(ruleDefn.getUpperLimit(), getUpperLimit()) &&
        CommonUtils.isEqual(ruleDefn.getBitWise(), getBitWise()) &&
        CommonUtils.isEqual(ruleDefn.isExactMatch(), isExactMatch()) &&
        (ApicUtil.compareStringAndNum(getRefValueDisplayString(), ruleDefn.getRefValueDisplayString()) == 0) &&
        CommonUtils.isEqual(ruleDefn.getUnit(), getUnit()) &&
   ...
   * @return
   */
  private boolean complexType(final RuleDefinition ruleDefn) {
    return CommonUtils.isEqual(ruleDefn.getLowerLimit(), getLowerLimit()) &&
        CommonUtils.isEqual(ruleDefn.getUpperLimit(), getUpperLimit()) &&
        CommonUtils.isEqual(ruleDefn.getBitWise(), getBitWise()) &&
        CommonUtils.isEqual(ruleDefn.isExactMatch(), isExactMatch()) &&
        CommonUtils.isEqual(ruleDefn.getRefValueDisplayString(), getRefValueDisplayString()) &&
        CommonUtils.isEqual(ruleDefn.getUnit(), getUnit()) &&
        CommonUtils.isEqual(ruleDefn.getReviewMethod(), getReviewMethod()) && (compareRefValueCmplxType(ruleDefn)) &&
        (ruleDefn.isDefaultRule() == isDefaultRule());
  }
   ...
   */
  public void setDefaultRule(final CDRRule defaultRule) {
